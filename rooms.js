...
function publicState(room) {
  return {
    ...
    players: room.players.map((p) => ({ id: p.id, name: p.name, connected: p.connected, cards: p.cards, threes: p.threes })),
...
export const roomEngine = {
  create({ hostId, name, setCode, setName, lang, alcoholFree }) {
    let code;
    do { code = makeCode(); } while (rooms.has(code));
    const room = newRoom(code, hostId, { setCode, setName, lang, alcoholFree });
    room.players.push({ id: hostId, name: cleanName(name), connected: true, cards: 0, threes: 0 });
    rooms.set(code, room);
    return room;
  },
  ...
  action(code, playerId, type, payload) {
    ...
    switch (type) {
      ...
      case "rule": {
        ...
        if (txt) room.houseRules.push(txt);
        return { room };
      }
      case "next": {
        if (!isTurn) return { error: "Niet jouw beurt" };
